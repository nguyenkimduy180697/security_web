...
#[AsCommand('cms:plugin:install-from-marketplace', 'Install a plugin from https://marketplace.cms.fsofts.com')]
class PluginInstallFromMarketplaceCommand extends Command implements PromptsForMissingInput
{
    ...
    public function handle(MarketplaceService $marketplaceService): int
    {
        ...
        if (version_compare($version, get_core_version(), '>')) {
            $this->components->error(trans('libs/plugin-management::marketplace.minimum_core_version_error', compact('version')));
