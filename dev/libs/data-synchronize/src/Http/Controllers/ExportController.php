            ...
            if (method_exists($exporter, 'getCounters')) {
                ...
                if ($totalItems > 10000 && $request->input('format') === 'xlsx') {
                    return $this
                        ->httpResponse()
                        ->setError()
                        ->setMessage(trans('libs/data-synchronize::data-synchronize.export.excel_not_supported_for_large_exports', ['count' => number_format($totalItems)]));
                }
            }
                    ...
                    foreach ($columns as $column) {
                        ...
                        switch ($columnName) {
                            ...
                            case 'taxes':
                                ...
                                break;
                            case 'image':
                                $row[] = AppMedia::getImageUrl($product->image);

                                break;
                            case 'images':
                                $row[] = collect($product->images)->map(fn ($value) => AppMedia::getImageUrl($value))->implode(',');
                            ...
                            foreach ($columns as $column) {
                                ...
                                switch ($columnName) {
                                    ...
                                    case 'quantity':
                                        ...
                                        break;
                                    case 'image':
                                        $varRow[] = AppMedia::getImageUrl($variation->product->image);

                                        break;
                                    case 'images':
                                        $varRow[] = collect($variation->product->images)->map(fn ($value) => AppMedia::getImageUrl($value))->implode(',');
