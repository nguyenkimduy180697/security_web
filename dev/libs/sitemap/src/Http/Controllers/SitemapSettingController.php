...
namespace Dev\Sitemap\Http\Controllers;
...
use Dev\Sitemap\Forms\Settings\SitemapSettingForm;
...
class SitemapSettingController extends SettingController
{
    ...
    {
        $this->pageTitle(trans('libs/sitemap::sitemap.settings.title'));
    ...
    public function generateKey(IndexNowService $indexNowService): JsonResponse
    {
        try {
            ...
            $message = trans('libs/sitemap::sitemap.settings.api_key_generated');

            if ($keyFileExists && $keyFileValid) {
                $message .= ' ' . trans('libs/sitemap::sitemap.settings.key_file_created');
            } elseif ($keyFileExists && ! $keyFileValid) {
                $message .= ' ' . trans('libs/sitemap::sitemap.settings.key_file_created_invalid');
            } else {
                $message .= ' ' . trans('libs/sitemap::sitemap.settings.key_file_creation_failed');
            }
            ...
            return response()->json([
                'success' => false,
                'message' => trans('libs/sitemap::sitemap.settings.generate_key_error'),
                'error' => $e->getMessage(),
            ], 500);
        }
    }
    ...
    public function createKeyFile(IndexNowService $indexNowService): JsonResponse
    {
        try {
            ...
            if (! $apiKey) {
                return response()->json([
                    'success' => false,
                    'message' => trans('libs/sitemap::sitemap.settings.no_api_key_to_create_file'),
                ], 400);
            }
            ...
            if ($success && $keyFileValid) {
                $message = trans('libs/sitemap::sitemap.settings.key_file_created_successfully');
            } elseif ($keyFileExists && ! $keyFileValid) {
                $message = trans('libs/sitemap::sitemap.settings.key_file_created_but_invalid');
            } else {
                $message = trans('libs/sitemap::sitemap.settings.key_file_creation_failed_detailed');
            }
            ...
            return response()->json([
                'success' => false,
                'message' => trans('libs/sitemap::sitemap.settings.key_file_creation_error'),
                'error' => $e->getMessage(),
            ], 500);
        }
    }
