    ...
    <div class="row g-2">
        <div class="col">
            <input
                ...
                name="api_key"
                value="{{ $apiKey }}"
                placeholder="{{ trans('libs/api::api.api_key_placeholder') }}"
                autocomplete="off"
                @if(!empty($apiKey)) readonly @endif
            >
        </div>
        <div class="col-auto">
            @include('libs/api::settings.partials.api-key-actions', ['apiKey' => $apiKey])
        </div>
    </div>
    
    <div class="form-text">
        {{ trans('libs/api::api.api_key_description') }}
    </div>
