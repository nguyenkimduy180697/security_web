...
export class MediaList {
    constructor() {
        this.group = {}
        this.group.list = $('#app_media_items_list').html()
        this.group.tiles = $('#app_media_items_tiles').html()

        this.item = {}
        this.item.list = $('#app_media_items_list_element').html()
        this.item.tiles = $('#app_media_items_tiles_element').html()

        this.$groupContainer = $('.app-media-items')
    }
    ...
    renderData(data, reload = false, load_more_file = false) {
        ...
        if (load_more_file && this.$groupContainer.find('.app-media-grid ul').length > 0) {
            $itemsWrapper = this.$groupContainer.find('.app-media-grid ul')
        }

        if (Helpers.size(data.folders) > 0 || Helpers.size(data.files) > 0 || load_more_file) {
            $('.app-media-items').addClass('has-items')
        } else {
            $('.app-media-items').removeClass('has-items')
        }
        ...
        Helpers.forEach(data.folders, (value) => {
            ...
            $item.find('.app-media-thumbnail').css('color', value.color)
