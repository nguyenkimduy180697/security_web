...
class InsertWatermarkCommand extends Command
{
    public function handle(): int
    {
        ...
        if (! setting('media_watermark_enabled', AppMedia::getConfig('watermark.enabled'))) {
            $this->components->error('Watermark is not enabled!');

            return self::FAILURE;
        }

        $watermarkImage = setting('media_watermark_source', AppMedia::getConfig('watermark.source'));
        ...
        $watermarkPath = AppMedia::getRealPath($watermarkImage);
        ...
        foreach ($files as $file) {
            ...
            try {
                ...
                if (empty($folderIds) || in_array($file->folder_id, $folderIds)) {
                    AppMedia::insertWatermark($file->url);
                }
