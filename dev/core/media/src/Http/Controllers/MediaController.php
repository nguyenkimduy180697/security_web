...
class MediaController extends BaseController
{
    ...
    public function postGlobalActions(Request $request, ThumbnailService $thumbnailService)
    {
        ...
        switch ($type) {
            ...
            case 'move':
                ...
                if ($error) {
                    $response = AppMedia::responseError(trans('core/media::media.move_error'));

                    break;
                }

                $response = AppMedia::responseSuccess([], trans('core/media::media.move_success'));
            ...
            case 'add_recent':
                ...
                if (empty($itemId)) {
                    $response = AppMedia::responseError('Invalid item ID');
                ...
                $response = AppMedia::responseSuccess([], trans('core/media::media.add_recent_success'));
    ...
    protected function moveFile(MediaFile $file, int|string|null $newFolderId = null)
    {
        ...
        $oldPath = AppMedia::getRealPath($file->url);
        $newFolderPath = AppMedia::getRealPath($this->folderRepository->getFullPath($newFolderId));
    ...
    public function download(Request $request)
    {
        ...
        if (count($items) == 1 && ! $items[0]['is_folder']) {
            ...
            if (! empty($file) && $file->type != 'video') {
                return AppMedia::responseDownloadFile($file->url);
            }
        } else {
