...
class MediaImageRule implements ValidationRule
{
    ...
    {
        if (AppMedia::isUsingCloud() || (is_string($value) && Str::startsWith($value, ['http', 'https']))) {
            if (strlen($value) > 190) {
        ...
        try {
            $file = $value instanceof UploadedFile ? $value : new UploadedFile(AppMedia::getRealPath($value), $value);
