...
class BaseSeeder extends Seeder
{
    ...
    protected function filePath(string $path, ?string $basePath = null): string
    {
        ...
        if (File::exists($filePath)) {
            try {
                $uploadedFile = AppMedia::uploadFromPath($filePath, 0, dirname($path));
                if (isset($uploadedFile['data']['url']) && $uploadedFile) {
                    return str_replace(AppMedia::getUploadURL() . '/', '', $uploadedFile['data']['url']);
                }
            } catch (Throwable $exception) {
    ...
    protected function fileUrl(string $path, ?string $basePath = null, ?string $size = null)
    {
        ...
        if ($size) {
            $path = AppMedia::getImageUrl($path, $size);
