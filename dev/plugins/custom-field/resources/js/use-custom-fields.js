...
class UseCustomFields {
    ...
    handleCustomFields() {
        ...
        let initCustomFieldsBoxes = (boxes, $appendTo) => {
            boxes.forEach((box) => {
                ...
                if (box.type === 'date' || box.type === 'datetime') {
                    setTimeout(() => {
                        Apps.initDatePicker($skeleton.find('.meta-box-wrap .datepicker'))
                    }, 100)
                }
        ...
        let registerRepeaterFieldLine = (items, data, $appendTo) => {
            data.forEach((item) => {
                ...
                if (item.type === 'date' || item.type === 'datetime') {
                    setTimeout(() => {
                        Apps.initDatePicker($result.find('> .repeater-item-input .datepicker'))
                    }, 100)
                }
        ...
        this.$body.on('click', '.repeater-add-new-field', (event) => {
            ...
            setTimeout(() => {
                Apps.initDatePicker($groupWrapper.find('.datepicker'))
                if (jQuery().timepicker) {
