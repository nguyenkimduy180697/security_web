...
@section('content')
    ...
    <div class="row mt-5">
        <div class="col-md-9 offset-md-3">
            @foreach(array_keys(\Dev\Sms\Facades\Sms::getDrivers()) as $driver)
                @php
                    $driver = \Dev\Sms\Facades\Sms::driver($driver);
                @endphp
                ...
                <x-core::card @class(['sms-gateway', 'mb-2' => ! $loop->last])>
                    ...
                    <div class="text-end p-2 border-top">
                        ...
                        >
                            {{ trans('plugins/sms-gateway::sms.configure_button') }}
                        </x-core::button>
                        <x-core::button
                                type="button"
                                color="info"
                                ...
                                @style(['display: none' => $driver->isEnabled()])
                        >
                            {{ trans('plugins/sms-gateway::sms.activate_button') }}
                        </x-core::button>
                    </div>
                        ...
                        <x-core::card.body class="border-top">
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    ...
                                    >
                                        {{ trans('plugins/sms-gateway::sms.test_button') }}
                                    </x-core::button>
                                </div>
                            </div>
                        </x-core::card.body>
                        <x-core::card.footer>
                            <div class="btn-list justify-content-end">
                                <x-core::button
                                        ...
                                        :data-url="route('sms.gateways.change-status', $driver->getKey())"
                                >
                                    {{ trans('plugins/sms-gateway::sms.deactivate_button') }}
                                </x-core::button>
                                <x-core::button
                                        type="submit"
                                        color="primary"
                                        form="dev-sms-forms-{{ $driver->getKey() }}-gateway-form"
                                >
                                    {{ trans('plugins/sms-gateway::sms.save_button') }}
                                </x-core::button>
                            </div>
                        </x-core::card.footer>
                    </div>
...
@push('footer')
    <x-core::modal
            id="test-sms-modal"
            :title="trans('plugins/sms-gateway::sms.test_modal.title')"
            :form-action="route('sms.gateways.test')"
    >
        <input type="hidden" name="gateway" value="" />

        <x-core::form.text-input
                :label="trans('plugins/sms-gateway::sms.test_modal.to')"
                :placeholder="trans('plugins/sms-gateway::sms.test_modal.to_placeholder')"
                type="tel"
                name="to"
        />

        <x-core::form.textarea
                :label="trans('plugins/sms-gateway::sms.test_modal.message')"
                name="message"
