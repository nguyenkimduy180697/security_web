...
class HookServiceProvider extends ServiceProvider
{
    ...
    public function addBarcodeMetaBox(string $context, object $object): void
    {
        if ($context === 'advanced' && $object instanceof Product && $object->exists) {
            add_meta_box(
                ...
                trans('plugins/barcode-generator::barcode-generator.name'),
    ...
    public function renderBarcodeMetaBox(Product $product): string
    {
        if (! $product->barcode && ! $product->sku) {
            return '<p class="text-muted">' . trans('plugins/barcode-generator::barcode-generator.no_barcode_data') . '</p>';
        }
        ...
        try {
            ...
            if ($product->barcode) {
                $html .= trans('plugins/barcode-generator::barcode-generator.barcode_value', ['value' => $product->barcode]);
            } else {
                $html .= trans('plugins/barcode-generator::barcode-generator.sku_value', ['value' => $product->sku]);
            }
            ...
            $html .= '<i class="ti ti-printer"></i> ' . trans('plugins/barcode-generator::barcode-generator.print_label');
            $html .= '</a>';
            $html .= '</div>';
            $html .= '</div>';
            ...
            return '<p class="text-danger">' . trans('plugins/barcode-generator::barcode-generator.generation_error') . ': ' . $e->getMessage() . '</p>';
        }
    }
    ...
    public function addProductBarcodeButton(string $buttons, Product $product): string
    {
        ...
        $button = '<a href="' . route('barcode-generator.index', ['products[]' => $product->id]) . '" class="btn btn-info btn-sm" title="' . trans('plugins/barcode-generator::barcode-generator.generate_barcode') . '">';
