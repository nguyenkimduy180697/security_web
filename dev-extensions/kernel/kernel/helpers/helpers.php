...
if (! function_exists('apps_cache_get_key')) {
    ...
    function apps_cache_get_key(string $cacheKey = 'default', string|null $group = null)
    {
        try {
            ...
            if (blank($group)) {
                return Str::slug(env('APP_NAME')) . ":" . $cacheKey;
            }
            ...
            $cache_list = cache()->has($app_cache_key) ? cache($app_cache_key) : [];
            $cache_list[$group] = true; // Lưu tên group vào trong danh sách quản lí app cache
            ...
            if (!in_array($cacheKey, $groupData)) {
                ...
                array_push($groupData, $cacheKey); // Thêm phần tử mới vào cuối mảng
...
if (! function_exists('apps_cache_flush')) {
    ...
    function apps_cache_flush(
        ...
        try {
            if (blank($group)) {
                cache()->forget($cacheKey); // Nếu không truyền group thì là xoá một key
            } else {
                // Xoá toàn bộ key trong group đó.
                $groupCacheKey = md5("group_" . $group);
                $groupData = [];
                if (cache()->has($groupCacheKey)) {
                    $groupData = json_decode(cache($groupCacheKey), true) ?? [];
...
if (!function_exists('apps_cache_reset')) {
    ...
    function apps_cache_reset()
    {
        try {
            $app_cache_key = md5("app_data_cache_list");
            $cache_list = cache()->has($app_cache_key) ? cache($app_cache_key) : []; // Lấy danh sách group
